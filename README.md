...
[![Build Status](https://travis-ci.org/log0u7/php.svg?branch=master)](https://travis-ci.org/log0u7/php)
[![Docker Pulls](https://img.shields.io/docker/pulls/log0u7/php.svg)](https://hub.docker.com/r/log0u7/php)
[![Docker Stars](https://img.shields.io/docker/stars/log0u7/php.svg)](https://hub.docker.com/r/log0u7/php)
[![Docker Layers](https://images.microbadger.com/badges/image/log0u7/php.svg)](https://microbadger.com/images/log0u7/php)
...
- [Images based on `log0u7/php`](#images-based-on-deployphp)
- [Orchestration Actions](#orchestration-actions)

## Docker Images

❗For better reliability **only** use images with stability tags (`log0u7/alpine:7.4-X.X.X`) which correspond to [git tags](https://github.com/log0u7/alpine/releases).

About images:

- All images based on Alpine Linux
- Base image: [php](https://github.com/docker-library/php)
- [Travis CI builds](https://travis-ci.org/log0u7/php)
- [Docker Hub](https://hub.docker.com/r/log0u7/php)
...
- `sudo` allowed for all commands for `deploy` user
...
Same as `-dev` but the default user/group `deploy` has uid/gid `501`/`20`  to match the macOS default user/group ids.
...
| --------------------------------- | ------------------- |
| `GIT_USER_EMAIL`                  | `deploy@6admin.io`  |
...
Change `DEPLOY_USER_ID` and `DEPLOY_GROUP_ID` mainly for local dev version of images, if it matches with existing system user/group ids the latter will be deleted.
...
You can run SSHD with this image by changing the command to `sudo /usr/sbin/sshd -De` and mounting authorized public keys to `/home/log0u7/.ssh/authorized_keys`

## Adding SSH key

You can add a private SSH key to the container by mounting it to `/home/log0u7/.ssh/id_rsa`

## Users and permissions

Default container user is `deploy:deploy` (UID/GID `1000`). PHP-FPM runs from `www-data:www-data` user (UID/GID `82`) by default. User `deploy` is a part of `www-data` group.

Codebase volume `$APP_ROOT` (`/var/www/html`) owned by `deploy:deploy`. Files volume `$FILES_DIR` (`/mnt/files`) owned by `www-data:www-data` with `775` mode.

See https://github.com/log0u7/php/issues/22 for more details.

#### Helper scripts

- `files_chmod` – in case you need write access for `deploy` user to a file/dir generated by `www-data` on this volume run `sudo files_chmod [FILEPATH]` script (FILEPATH must be under `/mnt/files`), it will recursively change the mode to `ug=rwX,o=rX`

- `files_chown` – in case you manually uploaded files under `deploy` user to files volume and want to change the ownership of those files to `www-data` run `sudo files_chown [FILEPATH]` script (FILEPATH must be under `/mnt/files`), it will recursively change ownership to `www-data:www-data`
...
[_(7/Dockerfile)_]: https://github.com/log0u7/php/tree/master/7/Dockerfile

[7.x xdebug]: https://github.com/log0u7/php/tree/master/7/templates/docker-php-ext-xdebug.ini.tmpl
[7.x newrelic]: https://github.com/log0u7/php/tree/master/7/templates/docker-php-ext-newrelic.ini.tmpl
[7.4 session]: https://github.com/log0u7/php/tree/master/7/templates/docker-php-7.4.ini.tmpl
[7.3 session]: https://github.com/log0u7/php/tree/master/7/templates/docker-php-7.3.ini.tmpl
[7.2 session]: https://github.com/log0u7/php/tree/master/7/templates/docker-php-7.2.ini.tmpl
